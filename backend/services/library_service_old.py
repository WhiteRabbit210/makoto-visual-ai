...
from services.database import library_index_table, LibraryQuery

class LibraryService:
    @staticmethod
    def initialize_sample_data():
        """サンプルデータの初期化"""
        if len(library_index_table.all()) == 0:
            # ルートフォルダ
            root_folder = {
                'id': 'root',
                'name': '全ライブラリ',
            ...
            library_index_table.insert(root_folder)
            ...
            for folder_data in sample_folders:
                ...
                library_index_table.insert(folder)
                ...
                if folder_data['name'] == 'MAKOTO操作説明':
                    for i in range(1, 4):
                        ...
                        library_index_table.insert(file_item)

    @staticmethod
    def get_items(parent_id: Optional[str] = None):
        """アイテムを取得"""
        if parent_id:
            return library_index_table.search(LibraryQuery.parent_id == parent_id)
        return library_index_table.all()
    
    @staticmethod
    def get_item(item_id: str):
        """特定のアイテムを取得"""
        items = library_index_table.search(LibraryQuery.id == item_id)
    ...
    def delete_item(item_id: str):
        """アイテムとその子要素を削除"""
        # 子要素を取得
        children = library_index_table.search(LibraryQuery.parent_id == item_id)
        ...
        # アイテム自体を削除
        library_index_table.remove(LibraryQuery.id == item_id)